...
public class AnswerServiceImpl extends ServiceImpl<AnswerMapper,Answer> implements AnswerService {
     ...
     */
    @Override
    public ResultSet<Answer> GetAnswersByDate(String topicId, Date date, int start, int limit) {
        PageHelper.offsetPage(start,limit,true);
        QueryWrapper<Answer> wrapper = new QueryWrapper<>();

        wrapper.select("no","substring(content,1,300) as content","conclusion","likes","recommend","topic_id as topicId","creator","creator_avat as creatorAvat","createBy")
       ...
       // return  this.baseMapper.queryAnsAndRecByDate(topicId,date);
        return resultSet;
    }
     ...
     */
    @Override
    public ResultSet<Answer> GetAdvanceAnsByTopicId(String topicId, Date date, int likes, int recoms, int start, int limit) {
        PageHelper.offsetPage(start,limit,true);
        QueryWrapper<Answer> wrapper = new QueryWrapper<>();
        ...
        wrapper.select("no","substring(content,1,300) as content","conclusion","likes","recommend","topic_id as topicId","creator","creator_avat as creatorAvat","createBy")
                .lambda().eq(Answer::getTopicId,topicId)
                .le(Answer::getCreateDat,date)
                .ge(Answer::getLikes,likes)
                .ge(Answer::getRecommend,recoms)
                .eq(Answer::getIsDel,0)
                .orderByDesc(Answer::getCreateDat);
        List<Answer> list = this.baseMapper.selectList(wrapper);
        PageInfo<Answer> pageInfo = new PageInfo<>(list);
