...
public interface CommentMapper extends BaseMapper<Comment> {
    @Select("SELECT id ,content,root_id,reply_id,reply_name,creator,creator_sex,creator_avat,create_by,create_dat " +
            "FROM t_pro_comment " +
            "WHERE root_id = #{id} " +
            "AND create_dat <=#{date} " +
            "AND is_del=0 " +
    ...
    List<Comment> queryNewComment(Map map);
    ...
    List<Comment> queryCommentByDate(@Param("rootId") Long id, @Param("date") Date date, @Param("start") int start,@Param("limit") int limit);
