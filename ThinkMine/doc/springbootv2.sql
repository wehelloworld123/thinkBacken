...
CREATE TABLE `t_deb_topic` (
  ...
  `answer` int  DEFAULT 0 COMMENT '回答数',
...
CREATE TABLE `t_deb_recom` (
  ...
  `parent_id` int DEFAULT -1 COMMENT '父节点',
  `creator` varchar(30)  DEFAULT NULL COMMENT '创建人',
  `create_by` varchar(35) DEFAULT NULL COMMENT '创建人Id',
  `create_dat` datetime DEFAULT NULL COMMENT '创建时间',
  `l_update_dat` datetime DEFAULT NULL COMMENT '最后更新时间',
...
CREATE TABLE `t_eva_report` (
  `no` varchar(20) NOT NULL  COMMENT '主键',
  `name` varchar(20) DEFAULT NULL  COMMENT '名称',
  `result` varchar(300) DEFAULT NULL COMMENT '测评结果',
  `feature` varchar(300) DEFAULT NULL  COMMENT '符合特征',
  `advice` varchar(300) DEFAULT NULL COMMENT '作品',
  `type` int DEFAULT 0 COMMENT '所属类型',
  `eval_id` varchar(32) DEFAULT NULL COMMENT '测评卷id',
  `create_by` varchar(32) DEFAULT NULL COMMENT '创建人',
  `create_dat` date DEFAULT NULL COMMENT '创建日期',
  `l_update_dat` datetime DEFAULT NULL COMMENT '最后更新时间',
  `is_del` int DEFAULT 0 COMMENT '是否删除',
  PRIMARY KEY (`no`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='测评报告表';
