...
CREATE TABLE `com_corporation` (
  ...
  `favorer` text DEFAULT 0 COMMENT '点赞人id',
...
-- Records of t_test
-- ----------------------------
DROP TABLE IF EXISTS `t_com_activity`;
CREATE TABLE `t_com_activity` (
  `uid` varchar(35) NOT NULL  COMMENT '主键',
  `subject` varchar(50) NOT NULL COMMENT '主题',
  `pic` varchar(30) DEFAULT NULL COMMENT '图片url',
  `dat` varchar(30) DEFAULT NULL COMMENT '时间',
  `address` varchar(30) DEFAULT NULL COMMENT '地址',
  `content_html` text DEFAULT NULL COMMENT '内容html',
  `views` int DEFAULT 0 COMMENT '点赞数',
  `concern` int DEFAULT 0 COMMENT '关注数',
...
DROP TABLE IF EXISTS `t_com_user_corp`;
CREATE TABLE `t_com_user_corp` (
...
CREATE TABLE `t_com_notice` (
  `id` bigint  NOT NULL AUTO_INCREMENT COMMENT '主键自增列',
  `title` varchar(30) NOT NULL COMMENT '开头标题',
  `content` varchar(300) NOT NULL COMMENT '内容',
  `link` varchar(100) DEFAULT NULL COMMENT '链接',
	`typ` int default 0 COMMENT '类型',
  `dat` varchar(20)  COMMENT '公布日期',
  `creator` varchar(10) DEFAULT NULL COMMENT '发布人',
   `create_by` varchar(35) DEFAULT NULL COMMENT '创建人',
  `create_dat` datetime DEFAULT NULL COMMENT '创建时间',
  `l_update_dat` datetime DEFAULT NULL COMMENT '最后更新时间',
  `is_del` int DEFAULT 0 COMMENT '是否删除',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3679 DEFAULT CHARSET=utf8 COMMENT='公告表'

DROP TABLE IF EXISTS `t_com_comment`;
...
DROP TABLE IF EXISTS `t_com_user_concern`;
CREATE TABLE `t_com_user_concern` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键自增列',
  `uid` varchar(35) DEFAULT NULL COMMENT '角色id',
  `aid` varchar(35) DEFAULT NULL COMMENT '活动id',
  `did` bigint DEFAULT 0 COMMENT '帖子id',
  `typ` int DEFAULT 1 COMMENT '关注状态',
  `create_by` varchar(35) DEFAULT NULL COMMENT '创建人',
  `create_dat` datetime DEFAULT NULL COMMENT '创建时间',
  `l_update_dat` datetime DEFAULT NULL COMMENT '最后更新时间',
  `is_del` int DEFAULT 0 COMMENT '是否删除',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='角色关注中间表';

DROP TABLE IF EXISTS `t_percom_info`;
CREATE TABLE `t_percom_info` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键自增列',
  `discuss` int DEFAULT 0 COMMENT '发布帖子数',
  `corporations` varchar(35) DEFAULT NULL COMMENT '社团数',
  `activities` bigint DEFAULT 0 COMMENT '参加活动数',
  `concern` int DEFAULT 1 COMMENT '关注贴数',
  `uid` varchar(35) DEFAULT NULL COMMENT '用户id',
  `create_by` varchar(35) DEFAULT NULL COMMENT '创建人',
  `create_dat` datetime DEFAULT NULL COMMENT '创建时间',
  `l_update_dat` datetime DEFAULT NULL COMMENT '最后更新时间',
  `is_del` int DEFAULT 0 COMMENT '是否删除',
...
CREATE TABLE `t_com_user_activity` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键自增列',
  `uid` varchar(35) DEFAULT NULL COMMENT '角色id',
  `cid` int DEFAULT NULL COMMENT '活动id',
  `state` int DEFAULT 1 COMMENT '关注状态',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户活动中间表';

-- 触发器，每更新用户申请社团记录，将社团人数+1,用户社团数+1
DELIMITER |
create trigger updateCorpNo
after update on t_com_user_corp
for each row
begin
 if
 (old.status != new.status) AND (new.status = 1)
	THEN
 update t_com_corporation set member_no = member_no+1 where id = new.cid;
 update t_percom_info set corporations = corporations+1 where uid = new.uid;
 end if;
  if
 (old.status != new.status) AND (new.status = -1)
	THEN
...
-- 触发器，每插入用户关注记录，用户关注数+1
DELIMITER |
create trigger updateConNo
after insert on t_com_user_concern
for each row
begin
 update t_percom_info set concern = concern+1 where uid = new.uid;
end
|

-- 触发器，每更新用户关注记录，用户关注数-1
DELIMITER |
create trigger delCorpNo
after update on t_com_user_corp
for each row
begin
  if
 (old.is_del != new.is_del) AND (new.is_del = 1)
	THEN
 update t_percom_info set concern = concern-1 where uid = new.uid;
 end if;
end
...
-- 触发器，用户每发布一个交流贴，用户帖子数+1
DELIMITER |
create trigger updateDissNo
after insert on t_com_discuss
for each row
begin
 update t_percom_info set discuss = discuss+1 where uid = new.create_by;
end
|

-- 触发器，用户每删除一个交流贴，用户帖子数-1
DELIMITER |
create trigger delDissNo
after update on t_com_discuss
for each row
begin
  if
 (old.is_del != new.is_del) AND (new.is_del = 1)
